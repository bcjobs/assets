...
(function($) {
	...
	$.fn.jsonSuggest = function(settings) {
		var defaults = {  
				...
				onSelect: undefined,
				width: undefined,
                textPropertyName: 'text' // 2013-01-11 (Johnny Customized): added textPropertyName so that this is now configurable
			},
			getJSONTimeout;
		...
		return this.each(function() {
			...
			*/
			function selectResultItem(item) {
			    obj.val(item[settings.textPropertyName]); // 2013-01-11 (Johnny Customized): replaced hard-coded text property with configurable textPropertyName.
				$(results).html('').hide();
			...
			*/
			function setHoverClass(el) {
				$('li a', results).removeClass('jsonsuggest_state_hover');
				if (el) {
					$('a', el).addClass('jsonsuggest_state_hover');
				}
			...
			*/
			function buildResults(resultObjects, filterTxt) {
				...
				for (i = 0; i < resultObjects.length; i += 1) {
					var item = $('<li />'),
						text = resultObjects[i][settings.textPropertyName]; // 2013-01-11 (Johnny Customized): replaced hard-coded text property with configurable textPropertyName.
						
					if (settings.highlightMatches === true) {
						text = text.replace(filterPatt, '<strong>$1</strong>');
					}
					
					$(item).append('<a>' + text + '</a>');
					...
					$(item).addClass('jsonsuggest_menu_item').
			...
			function runSuggest(e) {	
				var search = function(searchData) {
					...
					for (i = 0; i < searchData.length; i += 1) {
					    // 2013-01-11 (Johnny Customized): replaced hard-coded text property with configurable textPropertyName.
					    if (filterPatt.test(searchData[i][settings.textPropertyName]) === bMatch) {
				...
				if (settings.data && settings.data.length) {
					...
					$(results).html('<li class="jsonsuggest_menu_item ajaxSearching"><a>Searching...</a></li>').
						show().css('height', 'auto');
					
					getJSONTimeout = window.clearTimeout(getJSONTimeout);
					getJSONTimeout = window.setTimeout(function () {
                        // 2013-01-11 (Johnny Customized): changed query string parameter from 'search' to 'q'
						$.getJSON(settings.url, {q: text}, function(data) {
							if (data) {
			...
			*/
			...
			$(results).addClass('jsonSuggest jsonsuggest_autocomplete jsonsuggest_menu jsonsuggest_widget_content').
				attr('role', 'listbox').
				css({
			...
			obj.after(results).
				keyup(keyListener).
                keypress(function (e) { // 2013-01-11 (Johnny Customized): prevent form from submitting
                    if (e.which === 13)
